...
class gradingform_rubric_renderer extends plugin_renderer_base {
    ...
    public function level_template($mode, $options, $elementname = '{NAME}', $criterionid = '{CRITERION-id}', $level = null) {
        ...
        // check if student has self assessed
        if($DB->record_exists('gradingform_rubric_self', array('userid' => $USER->id, 'levelid' => $level['id']))) {
			if($options['allowselfassessment']) {
	        	$tdattributes['class'] .= ' self_checked';
			}
    ...
    protected function rubric_template($mode, $options, $elementname, $criteriastr) {
		...
		if($options['allowselfassessment']) {
			$rubrictemplate .= '<table width="100%" class="gradingform_rubric"><tbody class="criterion"><tr class="levels"><td>Key:</td><td class="level key currentchecked">Currently saved teacher assessed grade</td><td class="level checked key">Teacher assessed grade</td><td class="level self_checked key">Self assessed grade</td></tr></tbody></table>';
		}
